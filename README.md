...
| 명령어 | 설명 |
|---|---|
| `/망원밥` | 팀 탐험 현황 + 오늘의 추천 식당 3곳 |
| `/망원밥 한식` | 카테고리 필터 (한식/일식/중식/양식/카페/분식 등) |
| `/망원밥 방문 할매국밥` | 방문 기록. 첫 발견이면 채널에 공지 + 리뷰 모달 안내 |
| `/망원밥 리뷰 할매국밥` | 리뷰 작성 모달 (별점/코멘트/태그) |
| `/망원밥 탐험` | 내 개인 탐험 현황 + 단골 + 팀 내 순위 |
| `/망원밥 지도` | 팀 전체 탐험 현황 + 탐험왕/첫발견왕/단골왕 |
